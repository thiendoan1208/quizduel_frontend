...
import { addUserToWaitingQueue } from "@/service/game/before-game";
...
function GameSetting({
  ...
  const { mutate: findMatchMutate } = useMutation({
    mutationKey: ["findMatchMutate"],
    mutationFn: (userMatchInfo: UserMatchInfo) =>
      addUserToWaitingQueue(userMatchInfo),
  });
  ...
  const modifyIUserMatchInfo = () => {
    const newUserMatchInfo: UserMatchInfo = {
      userInfo: {
        name: user.name,
        win: user.win,
        loss: user.loss,
    ...
    return newUserMatchInfo;
  };

  const handleFindMatch = () => {
    if (userTopicName.length > NUMBER_OF_TOPIC) {
      toast.error("Chỉ được chọn tối đa 3 chủ đề");
    } else if (userTopicName.length < MINIMUM_OF_TOPIC) {
      toast.error("Chọn ít nhất 1 chủ đề");
    } else {
      const userMatchInfo = modifyIUserMatchInfo();

      showWaitingCard();
      findMatchMutate(userMatchInfo);
    }
  };
  ...
  return (
    <>
      <div
        className={`${
          isOpen ? "fixed" : "hidden"
        } md:min-w-[500px] w-full h-screen top-0 left-0 flex items-center justify-center`}
      >
        <Card className="w-[400px] md:w-[500px] max-h-[700px] md:max-h-[1000px] overflow-auto bg-black border-green-700 text-white px-4 mx-2 md:mx-0">
          <div>
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-semibold text-green-500 flex flex-col md:block">
                Chọn chế độ:
      ...
      <div className={isWaiting ? "block" : "hidden"}>
        <Waiting
          isWaiting={isWaiting}
          setIsWaiting={setIsWaiting}
          getUserMatchInfo={modifyIUserMatchInfo}
        />
      </div>
    </>
  );
