...
import { createQuizByTopic } from "@/service/game/during-game";
...
export default function QuizBackground() {
  ...
  const { data: quizInfo } = useQuery({
    queryKey: ["allQuizInfo", matchInfo.users],
    queryFn: async () => {
      const userMatchInfo = modifyUser();
      const response = await createQuizByTopic(userMatchInfo);
      return response;
    },
    staleTime: 5 * 60 * 60,
    enabled: !!user,
              ...
              <div className="flex flex-col md:grid md:grid-cols-2 w-full gap-2 justify-between mt-4">
                <div className="flex-1 md:col-span-1 bg-black/50 border border-green-600/60 rounded-xl text-left text-green-300 p-4 backdrop-blur-md shadow-[0_0_15px_rgba(0,255,150,0.3)]">
                  <p>
                    ...
                    {userQuery?.data[0]?.name || "Đang tải..."}
                  </p>
                  <p>
                    <span className="font-semibold text-green-400">
                      Thắng / Thua:
                    ...
                    <span className="text-green-500 font-bold">
                      {userQuery?.data[0]?.win ?? 0}
                    </span>{" "}
                    /{" "}
                    <span className="text-red-400 font-bold">
                      {userQuery?.data[0]?.loss ?? 0}
                    </span>
                  </p>
                </div>
                <div className="flex-1 md:col-span-1 bg-black/50 border border-green-600/60 rounded-xl text-left text-green-300 p-4 backdrop-blur-md shadow-[0_0_15px_rgba(0,255,150,0.3)]">
